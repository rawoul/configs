...
-- Common programs
screenlocker = "kwrapper4 kscreenlocker --forcelock"
terminal = "urxvtc"
...
for s = 1, screen.count() do
    ...
    for tagnumber = 1, 9 do
        ...
        awful.tag.setmwfact(0.33, tags[s][tagnumber])
        awful.tag.setncol(3, tags[s][tagnumber])
...
clockwidget.text = bold(awesome.release)
clockformat = "%a %d.%m.%Y %T"

-- CPU
cpuwidget = widget({ type = "textbox", align = "right" })
wicked.register(cpuwidget, "cpu", function (widget, args)
    return widget_set("CPU", args[1] .. "%")
end, 3)

-- Memory
memwidget = widget({ type = "textbox", align = "right" })
wicked.register(memwidget, "mem", widget_set("RAM", "$2" .. hilight("/") .. "$3MB"), 10)
...
globalkeys = awful.util.table.join(
    ...
    -- Screen Lock
    awful.key({ modkey            }, "Pause", function () awful.util.spawn(screenlocker) end),
...
awful.hooks.timer.register(1, function ()
    clockwidget.text = os.date(clockformat)
end)
